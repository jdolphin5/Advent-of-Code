        ...
        for (let i = 0; i < lineArr.length; i++) {
            for (let j = 0; j < lineArr[i].length; j++) {
                if (lineArr[i].charAt(j) === "0") {
                    const add: number = recur(lineArr, i, j, "0", new Set());
                    console.log(add);
                    result += add;
                }
            }
...
const recur = (
    ...
    if (i - 1 >= 0 && lineArr[i - 1].charAt(j) === nextNum[lastNum]) {
        ret += recur(lineArr, i - 1, j, nextNum[lastNum], new Set(visited));
    }
    ...
    if (j - 1 >= 0 && lineArr[i].charAt(j - 1) === nextNum[lastNum]) {
        ret += recur(lineArr, i, j - 1, nextNum[lastNum], new Set(visited));
    }
